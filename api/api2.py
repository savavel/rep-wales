...
app.config['TOKEN_SECRET'] = "123test"
# extensions
db = SQLAlchemy(app)
print "in"
auth = HTTPBasicAuth()
...
def new_user():
    ...
    #user = User(firstname=firstname,lastname=lastname,password=passwd_hash,email=email,mnemonic=mnemonic,address=address,user_type=userType)
    print "after conn"
    #db.session.add(user)
    #db.session.commit()
...
def user_auth():
    ...
    data = db.engine.execute("SELECT email, password FROM accounts WHERE email='"+str(email)+"';")
    names = []
    for row in data:
        for x in row:
            names.append(str(x))
    if len(names) == 0 or not verify_password(names[1], password):
        return (jsonify("Username or Password is wrong"),401)
    else:
        #db.engine.execute("UPDATE accounts SET token='"+ token +"' WHERE id="+names[0]+";";
...
def get_user_info():
    ...
    try:
        ...
        email = payload['sub']
	data = db.engine.execute("SELECT * FROM accounts WHERE email='"+str(email)+"';")
	names = []
        for row in data:
            for x in row:
                names.append(str(x))
        if len(names) == 0:
            return (jsonify("Should not happen"), 500)
        else:
            return (jsonify({"name": names[1], "lastname": names[2], "email": names[3], "password": names[4],"mnemonic": names[5],"address": names[6], "type": names[7]}), 200)

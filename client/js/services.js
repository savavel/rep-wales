...
//curl -i -X GET -H "Content-Type: application/json" -d '{"email":"test@test","password":"test"}' http://127.0.0.1:5000/api/user
	...
	$http({
		...
		{
			$log.log('client')
			deferred.reject('You do not have priviliges to use the dashboard.');
		}
		else {
...
.service('TransactService', function($q, $http, $log, walletSettings) {
    return {
        sendTransaction: function(assetPath, amount) {
	...
	$http({
		...
		data: {
			...
			"destination": "XhKEBUqQo9t4RqZDHPZFVHsbpRrkTSx4BF",
			"asset": assetPath,
			"amount": amount
			}
	    }).then(function (response) {
...
.service('CreateAssetService', function($q, $http, $log, walletSettings) {
    return {
        createAsset: function(name, nameShort, iconUrl, amount, price) {
	...
	$http({
		...
		url : "http://ec2-34-250-36-85.eu-west-1.compute.amazonaws.com/api/asset",
		data: {
			"mnemonic": "home poverty easily vocal devote broken clever lion inner mass shell lava",
